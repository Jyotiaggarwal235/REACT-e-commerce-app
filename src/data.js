
export const storeProducts = [
    {
      id: 1,
      title: "LQDCELL Tension one8 Training Shoes",
      img: "img/Shoes_1.png",
      price: 35000,
      company: "PUMA",
      info:  "Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 2,
      title: "Joyride Dual Run Marathon Running Shoes/Sneakers",
      img: "img/Shoes_2.png",
      price: 10000,
      company: "PUMA",
      info:  "Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 3,
      title: "RS-2K Internet Exploring Sneakers",
      img: "img/Shoes_3.png",
      price: 60000,
      company: "PUMA",
      info:  "Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 4,
      title: "HTC 10 - Black",
      img: "img/Shoes_4.png",
      price: 39999,
      company: "v",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 5,
      title: "New Fashion Sport Sneaker for Men Shoes",
      img: "img/Shoes_5.png",
      price: 56908,
      company: "PUMA",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    
    {
      id: 6,
      title: "RS-2K Internet Exploring Sneakers",
      img: "img/Shoes_6.png",
      price: 56999,
      company: "PUMA",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 7,
      title: "New Fashion Sport Sneaker for Men Shoes",
      img: "img/Shoes_7.png",
      price: 39999,
      company: "PUMA",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 8,
      title: "New Fashion Sport Sneaker for Men Shoes",
      img: "img/Shoes_8.png",
      price: 20000,
      company: "PUMA",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 9,
      title: "New Fashion Sport Sneaker for Men Shoess",
      img: "img/Shoes_9.png",
      price: 100000,
      company: "PUMA",
      info:  "Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 10,
      title: "New Fashion Sport Sneaker for Men Shoes ",
      img: "img/Shoes_10.png",
      price: 120200,
      company: "PUMA",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 11,
      title: "New Fashion Sport Sneaker for Men Shoes",
      img: "img/Shoes_11.png",
      price: 89562,
      company: "NIKE",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 12,
      title: "Nike Jordan Zoom '92 Basketball Shoes/Sneakers",
      img: "img/Shoes_12.png",
      price: 8950,
      company: "NIKE",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
     
      count: 0,
      total: 0
    },
    {
      id: 13,
      title: "New Fashion Sport Sneaker for Men Shoes",
      img: "img/Shoes_13.png",
      price: 18500,
      company: "NIKE",
      info:"Nike Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    
    {
      id: 14,
      title: "Joyride Dual Run Marathon Running Shoes/Sneakers",
      img: "img/Shoes_1.png",
      price: 5622,
      company: "PUMA",
      info:"Puma Crater Impact is part of our sustainability journey to transform trash into shoes that tread a little lighter.Made from at least 25% recycled material by weight, it brings in unique design choices that reduce waste when compared with traditional methods—like an embroidered Swoosh, efficient overlays and of course its Crater foam midsole.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 15,
      title: "Joyride Dual Run Marathon Running Shoes/Sneakers",
      img: "img/product-7.png",
      price: 30000,
      company: "PUMA",
      info:"Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
      inCart: false,
      count: 0,
      total: 0
    },
    {
      id: 16,
      title: "New Fashion Sport Sneaker for Men Shoes",
      img: "img/Shoes_1.png",
      price: 200,
      company: "PUMA",
      info:
        "Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
      inCart: false,
      count: 0,
      total: 0
    }
  ];
  
  export const detailProduct =   {
    id: 1,
    title: "New Fashion Sport Sneaker for Men Shoes",
    img: "img/Shoes_11.png",
    price: 100000,
    company: "PUMA",
    info:
    "Kevin Durant lurks on the wing, waiting for the right time to strike before slicing his way through defences. The KD14 is designed to help versatile, relentless players like KD feel fresh all game long. Multi-layer mesh and a midfoot strap help reduce the foot's movements inside the shoe. Full-length Zoom Air cushioning plus Cushlon foam give back energy for lasting performance. This EP version uses an extra-durable sole that's ideal for outdoor courts.",
  inCart: false,
    count: 0,
    total: 0
  }
  